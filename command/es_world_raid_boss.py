...
async def handle(bot: Bot, event: Event, args: Message = CommandArg()):
    try:
        ...
        data = await load_json_data(group_id, command_name="es世界BOSS信息")
        ...
        # 验证BOSS是否存在于世界讨伐中的函数
        def get_valid_world_boss_data(target_hero_id):
            for boss in data["world_raid_boss"]["json"]:
        ...
        if hero_ids:
            for hid in hero_ids:
                if b_data := get_valid_world_boss_data(hid):
                    boss_data = b_data
                    hero_id = hid
                    break
        ...
        if not hero_id:
            ...
            if matches:
                ...
                # 在模糊匹配中也寻找有效的世界讨伐BOSS ID
                matched_hero_id = None
                for hid in matched_hero_ids:
                    if get_valid_world_boss_data(hid):
                        matched_hero_id = hid
                        break
        ...
        if not hero_data:
            await es_world_raid_boss.finish("未找到该BOSS的基础信息")
