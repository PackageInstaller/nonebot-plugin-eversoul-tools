...
async def handle(bot: Bot, event: Event, matched: Tuple[Any, ...] = RegexGroup()):
    try:
        ...
        for stage in data["stage"]["json"]:
            if (
                stage.get("stage_type") == GATE_TYPE_MAPPING[gate_type]
                and stage.get("stage_no") == stage_no
            ):
                ...
                for team in battle_teams:
                    ...
                    for i in range(1, 6):
                        ...
                        grade_data = await get_string_by_type(
                            data, "system", team.get(f"hero_grade_{i}")
                        )
                        ...
                        if equip_no := team.get(f"hero_equip_{i}"):
                            equip_data = next(
                                (
                                    e
                                    for e in data["stage_equip"]["json"]
                                    if e["no"] == equip_no
                                ),
                                None,
                            )
