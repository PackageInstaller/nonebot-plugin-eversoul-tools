...
async def handle_unbind(bot: Bot, event: Event):
    ...
    if not user_accounts:
        await es_unbind.finish(
            message="您尚未绑定过账号，请使用 es绑定账号 命令进行绑定",
            reply_message=True,
        )
    ...
    for i, account in enumerate(user_accounts):
        if i < len(emoji_list):
            # 获取服务器名称
            server_name = SERVER_NAME_MAPPING.get(
                SERVER_NAME_REVERSE_MAPPING.get(account["app_id"], "未知"),
                account["app_id"],
            )
    ...
    unbind_votes[message_id] = {
        ...
        "emoji_to_index": {
            emoji_ids[i]: i for i in range(min(len(user_accounts), len(emoji_ids)))
        },  # 表情ID到索引的映射
    }
    ...
    for i in range(min(len(user_accounts), len(emoji_ids))):
        ...
        await bot.call_api(
            "set_msg_emoji_like", message_id=message_id, emoji_id=emoji_ids[i]
        )

    # 添加确认和取消表情
    await asyncio.sleep(0.2)
    await bot.call_api(
        "set_msg_emoji_like", message_id=message_id, emoji_id="10024"
    )  # 确认
    await asyncio.sleep(0.2)
    await bot.call_api(
        "set_msg_emoji_like", message_id=message_id, emoji_id="10060"
    )  # 取消
...
async def finalize_unbind(bot: Bot, event: Event, vote_data: Dict):
    ...
    for index in selected_indices:
        ...
        success = await EversoulUser.delete_specific_account(
            int(user_id), account["player_id"]
        )
        server_name = SERVER_NAME_MAPPING.get(
            SERVER_NAME_REVERSE_MAPPING.get(account["app_id"], "未知"),
            account["app_id"],
