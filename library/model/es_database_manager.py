...
class EversoulUser:
    ...
    async def add_user(cls, user_id: int, app_id: str, player_id: str) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(cls._db_path) as db:
                ...
                await db.commit()
                logger.info(
                    f"用户数据已保存: user_id={user_id}, app_id={app_id}, player_id={player_id}"
                )
            return True
        except Exception as e:
    ...
    async def get_user(cls, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with aiosqlite.connect(cls._db_path) as db:
                ...
                cursor = await db.execute(
                    "SELECT * FROM user WHERE user_id = ? LIMIT 1", (user_id,)
                )
    ...
    async def delete_specific_account(cls, user_id: int, player_id: str) -> bool:
        ...
        try:
            ...
            async with aiosqlite.connect(cls._db_path) as db:
                ...
                await db.commit()
                logger.info(
                    f"用户特定账号已删除: user_id={user_id}, player_id={player_id}"
                )
            return True
        except Exception as e:
    ...
    @classmethod
    async def update_coupon_history(
        cls, user_id: int, player_id: str, coupon_code: str, status: Dict[str, Any]
    ) -> bool:
        """更新用户的兑换码历史
        ...
        try:
            ...
            async with aiosqlite.connect(cls._db_path) as db:
                ...
                await db.commit()
                logger.info(
                    f"兑换码历史已更新: user_id={user_id}, player_id={player_id}, code={coupon_code}"
                )
            return True
        except Exception as e:
    ...
    async def update_server(
        cls, server_type: str, version: str, cdn_date: str = "", table_version: int = 0
    ) -> bool:
        """更新服务器状态
    ...
    @classmethod
    async def check_push_history(
        cls, server_type: str, version: str, table_version: int, group_id: int
    ) -> bool:
        """检查是否已经推送过指定版本
    ...
    async def add_push_history(
        ...
        try:
            ...
            async with aiosqlite.connect(cls._db_path) as db:
                ...
                await db.commit()
                logger.info(
                    f"推送历史已记录: {server_type} - {version} - 表版本{table_version} -> 群{group_id}"
                )
            return True
        except Exception as e:
